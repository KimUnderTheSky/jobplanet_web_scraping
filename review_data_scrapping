...
#viewCompaniesMenu > ul > li.viewInterviews > a
...
driver = webdriver.Chrome("셀레니움크롬드라이버 위치")
...
for i in range(page): 
    ...
    point = driver.find_elements_by_css_selector("div.bl_score") #페이지내 총 point 개수
    #승진기회및 가능성, 복지 및 급여, 업무와 삶의 균형, 사내문화, 경영진
    #페이지당 25개    
    ...
    for k, j in enumerate(point):
        ...
        if k%5 == 1:
            if a[7:9] == '20':
                list_possible.append("1점")
            elif a[7:9] == '40':
                list_possible.append("2점")
            elif a[7:9] == '60':
                list_possible.append("3점")
            elif a[7:9] == '80':
                list_possible.append("4점")
            else:
                list_possible.append("5점")
        elif k%5 == 2:
            if a[7:9] == '20':
                list_pay.append("1점")
            elif a[7:9] == '40':
            ...
            if a[7:9] == '20':
                list_workbal.append("1점")
            elif a[7:9] == '40':
                list_workbal.append("2점")
            elif a[7:9] == '60':
                list_workbal.append("3점")
            elif a[7:9] == '80':
                list_workbal.append("4점")
            else:
                list_workbal.append("5점")
        elif k%5 == 4:
            if a[7:9] == '20':
                list_culture.append("1점")
            elif a[7:9] == '40':
                list_culture.append("2점")
            elif a[7:9] == '60':
                list_culture.append("3점")
            elif a[7:9] == '80':
                list_culture.append("4점")
            else:
                list_culture.append("5점")
        else:
            if a[7:9] == '20':
                list_manage.append("1점")
            elif a[7:9] == '40':
                list_manage.append("2점")
            elif a[7:9] == '60':
                list_manage.append("3점")
            elif a[7:9] == '80':
                list_manage.append("4점")
            else:
                list_manage.append("5점")
...
total_data["복지 및 급여"] = pd.Series(list_possible)
total_data["업무와 삶의균형"] = pd.Series(list_pay)
total_data["사내문화"] = pd.Series(list_workbal)
total_data["경영진"] = pd.Series(list_culture)
total_data["승진기회 및 가능성"] = pd.Series(list_manage)
...
# step9.엑셀 형태로 저장하기
total_data.to_excel("하나은행리뷰데이터.xls" ,index=True)
